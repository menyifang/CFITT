function NNF_H = upsample(NNF_L, trgdistCur, srcdistCur, optS)
%
...
%关键点测试
% load prematch.mat;
% scaletimes = [NNF_H.imgH/optS.Htrg NNF_H.imgW/optS.Wtrg NNF_H.imgH2/optS.Hsrc NNF_H.imgW2/optS.Wsrc];
% scaletimes = repmat(scaletimes,size(prematch,1),1);
% prematch = round((prematch.*scaletimes)')-optS.pRad;
% prematch(:,find(any(prematch<1,1)))=[];
% trgind = sub2ind([NNF_H.imgH-optS.pSize+1, NNF_H.imgW-optS.pSize+1], prematch(2,:), prematch(1,:));
% srcind = sub2ind([NNF_H.imgH2-optS.pSize+1, NNF_H.imgW2-optS.pSize+1], prematch(4,:), prematch(3,:));
% % randInd(trgind)=srcind;
...
%关键点测试
% NNF_H.uvTform.data(1:2,trgind) = NNF_H.validPix.sub(:, srcind);
...
NNF_H.pSizeWeight.data = get_psize_weight(NNF_H.uvPix.color, pSizehist, NNF_H.pSizeWeight.LvlInd, optS);
